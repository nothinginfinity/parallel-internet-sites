/**
 * Cloudflare Worker — AFO audit-signup
 * AFO v1 dogfood launch. No payment. Founder coupons only.
 *
 * Routes:
 *   GET  /api/health              — liveness check
 *   GET  /api/gh-check            — dev: GitHub token diagnostics
 *   GET  /test                    — dev: manual test UI
 *   POST /api/audit-signup        — existing intake handler (UNCHANGED)
 *   GET  /results                 — snapshot results page
 *   POST /api/visibility-snapshot — NEW: free snapshot endpoint (Commit 7+)
 */

import { generatePrompts } from './prompt-generator.js';
import { renderResultsPage } from './results-page.js';
import { handleVisibilitySnapshot } from './visibility-snapshot.js';
...
export default {
  async fetch(request, env) {
    ...
    // Commit 7: New free snapshot endpoint
    if (url.pathname === '/api/visibility-snapshot' && request.method === 'POST') {
      return handleVisibilitySnapshot(request, env);
    }

    // CORS preflight support for both POST endpoints
...
// ---------------------------------------------------------------------------
// POST /api/audit-signup — UNCHANGED
// ---------------------------------------------------------------------------
...
async function handleAuditSignup(request, env) {
  ...
  catch { return json({ ok: false, error: 'Invalid website_url' }, 400); }
  ...
  if (env.TURNSTILE_SECRET && env.TURNSTILE_SECRET !== 'SKIP_IN_DEV') {
    ...
    if (!tsResult.success) return json({ ok: false, error: 'Turnstile verification failed' }, 400);
  }
  ...
  try {
    ...
    auditRequest = await createAuditRequest(env.DB, {
      customerId: customer.id, customerEmail: email, customerName: name,
      businessName, websiteUrl, couponCode: couponRaw, plan, source, launchPhase,
    });
  } catch (err) {
  ...
  if (couponRaw) {
    try {
      await logCouponRedemption(env.DB, {
        customerId: customer.id, auditRequestId: auditRequest.id, couponCode: couponRaw,
      });
    } catch (err) {
  ...
  return json({
    ...
    plan,
    _debug: { email: emailResult, github: githubResult },
  });
}
...
async function createGitHubIssue(env, { email, name, businessName, websiteUrl, plan, couponCode, auditRequestId }) {
  if (!env.GITHUB_TOKEN) return { ok: false, error: 'No GITHUB_TOKEN configured' };
  ...
  try {
    ...
    catch { return { ok: false, status: res.status, raw_response: rawText.slice(0, 300) }; }
